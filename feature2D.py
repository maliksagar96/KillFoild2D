...
def feature2D(img,Lambda,w,masscut=0,Imin=0,field=2,bandpass='bp'):
    ...
    cmask[int(cen),int(cen)]=0.0
    smask[int(cen),int(cen)]=0.0
    ...
    for i in range(0,nmax):
        ...
        tmp2 = (m[i]-suba[int(cen),int(ycen),i]+1e-6)
        e[i] = tmp/tmp2
